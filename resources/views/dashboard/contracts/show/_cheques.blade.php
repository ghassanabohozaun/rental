                    ...
                    <tbody style="font-size: 15px;">
                        @forelse($contract->cheques as $cheque)
                            <tr>
                                ...
                                <td class="py-2">
                                    <span class="font-weight-bold text-dark">{{ number_format($cheque->amount, 2) }}</span>
                                    <small class="text-muted ml-1">{!! currency() !!}</small>
                                </td>
                                <td class="py-2">{!! $cheque->due_date ? $cheque->due_date->format('Y-m-d') : '---' !!}</td>
                                <td class="py-2 text-center">
                                    <span
                                        class="badge badge-light-{{ $cheque->status_color }} badge-pill border-0 px-2 py-1">
                                ...
                                <td class="py-2">
                                    @if ($cheque->is_deposit)
                                        <span
                                            class="badge badge-light-warning badge-pill border-0 px-2 py-1">{!! __('cheques.insurance_cheques') !!}</span>
                                    @else
                                        <span
                                            class="badge badge-light-primary badge-pill border-0 px-2 py-1">{!! __('cheques.rent_cheques') !!}</span>
                                    @endif
                                </td>
                            </tr>
                        @empty
