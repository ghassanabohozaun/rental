...
<div class="row">
    <div class="col-md-12">
        <div class="premium-form-group">
            ...
            <input type="text" id="grace_period" name="contract_detail[grace_period]" class="form-control shadow-none"
                value="{!! old('contract_detail.grace_period') !!}" placeholder="{!! __('contracts.grace_period_placeholder') !!}">
        </div>
    </div>
</div>
...
<div class="row">
    <div class="col-md-12">
        ...
        <div id="clauses-container" class="sortable-clauses">
            <!-- Clauses will be rendered here via JS -->
        </div>
        
        <button type="button" class="btn btn-outline-primary mt-2" onclick="addNewEmptyClause()">
    ...
    function openClauseLibraryModal() {
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                if(res.length === 0) {
                    $('#library-content').html('<div class="alert alert-warning">{!! __('contracts.empty_library_message') !!}</div>');
                    return;
                }
