<li class="dropdown dropdown-notification nav-item" x-data="{ open: false }" @click.outside="open = false" wire:poll.20s="checkNotifications">
    ...
    <div class="dropdown-menu dropdown-menu-right navbar-dropdown preview-list" 
        ...
        <div class="dropdown-header-main pb-1">
            ...
            @if($unreadCount > 0)
                <span class="premium-badge-count cursor-pointer" wire:click="markAllAsRead" style="cursor: pointer;">
        ...
        <div class="px-0 pt-0 pb-1 text-center d-flex justify-content-center border-bottom">
            <ul class="nav premium-nav-tabs" id="headerNotificationTabs" role="tablist" style="transform: scale(0.85); transform-origin: center top; margin-bottom: -5px;">
                <li class="nav-item">
        ...
        <div class="scrollable-container media-list w-100 custom-scrollbar" style="max-height: 350px; overflow-y: auto;">
            @forelse($notifications as $notification)
                @php
                    ...
                    $bgClass = 'bg-' . $level;
                @endphp
                <a class="preview-item-premium" href="{{ route('dashboard.notifications.redirect', $notification->id) }}" style="{{ $isUnread ? 'background-color: rgba(102, 110, 232, 0.05); border-right: 3px solid #666ee8;' : 'transition: background-color 0.2s;' }}">
                    ...
                    <div class="preview-item-content-premium w-100">
                        <span class="subject">
                            {{ $title }}
                            @if($isUnread)
                                <span class="badge badge-pill badge-danger shadow-sm ml-1" style="font-size: 0.55rem; padding: 2px 5px;">{{ __('notifications.new') }}</span>
                            @endif
                        </span>
                        <span class="message-text d-block mb-1">{{ $message }}</span>
                        
                        @if(isset($data['action_livewire']) && isset($data['action_text']))
                            <div class="mt-2 mb-3">
                                <button wire:click.prevent="{{ $data['action_livewire'] }}" class="btn btn-sm btn-premium-add shadow-pulse" style="font-size: 0.75rem; padding: 0.2rem 0.6rem; line-height: 1.5;">
                                    {{ __($data['action_text']) }}
                                </button>
                            </div>
                        @endif
                        
                        <span class="time text-muted d-block mt-1" style="font-size: 0.7rem;"><i class="far fa-clock mr-1"></i> {{ $notification->created_at->diffForHumans() }}</span>
                    </div>
                </a>
            @empty
        ...
        <div class="dropdown-footer-premium">
            <a href="{{ route('dashboard.notifications') }}"><i class="fas fa-list-ul mr-1"></i> {{ __('notifications.view_all') }}</a>
        </div>
    </div>
</li>
