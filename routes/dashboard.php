...
use App\Http\Controllers\Dashboard\ChequesController;
use App\Http\Controllers\Dashboard\CompaniesController;
use App\Http\Controllers\Dashboard\CompanyBankAccountController;
...
use App\Http\Controllers\Dashboard\GuarantorsController;
use App\Http\Controllers\Dashboard\MaintenancesController;
    ...
    function () {
        ...
        Route::group(['middleware' => ['auth:web', 'checkLockScreen']], function () {
            ...
            Route::group(['middleware' => 'can:contracts_read'], function () {
                ...
                Route::get('/contracts/print/{id}', [ContractsController::class, 'print'])->name('contracts.print');
                
                // Contract Clause Templates (Library)
                Route::resource('contract_clause_templates', \App\Http\Controllers\Dashboard\ContractClauseTemplateController::class);
                Route::post('/contract_clause_templates/destroy/{id}', [\App\Http\Controllers\Dashboard\ContractClauseTemplateController::class, 'destroy'])->name('contract_clause_templates.destroy');
                Route::get('/api/contract_clauses', [\App\Http\Controllers\Dashboard\ContractClauseTemplateController::class, 'getCompanyClauses'])->name('contract_clauses.api');
            });
            // ########################################## cheques routes #############################################################
            ...
            Route::group(['middleware' => 'can:reports_properties', 'prefix' => 'reports/properties', 'as' => 'reports.properties.'], function () {
                Route::get('/', [PropertiesReportsController::class, 'index'])->name('index');
                Route::post('/export', [PropertiesReportsController::class, 'exportExcel'])->name('export.excel');
            });
        });
    },
);
